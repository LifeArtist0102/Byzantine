...
def _pages(evidence: Evidence) -> str:
    if evidence.printed_page_start:
        ...
        return (
            str(evidence.printed_page_start)
            if end == evidence.printed_page_start
            else f"{evidence.printed_page_start}-{end}"
        )
    if evidence.pdf_page_start:
        end = evidence.pdf_page_end or evidence.pdf_page_start
        suffix = (
            str(evidence.pdf_page_start)
            if end == evidence.pdf_page_start
            else f"{evidence.pdf_page_start}-{end}"
        )
        return f"PDF {suffix}"
    return ""
...
def format_gbt7714(evidence: Evidence) -> str:
    ...
    publication = ": ".join(
        ...
        for part in (
            evidence.publisher,
            str(evidence.publication_year) if evidence.publication_year else "",
        )
        if part
    )
    if publication:
