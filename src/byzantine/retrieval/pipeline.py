...
def _metadata_fallback(
    ...
    *,
    ...
    if not any(metadata_values(plan).values()) or len(strict) >= 3:
        return strict
    partial = _retrieve_once(
        _without_metadata(plan, partial=True),
        database=database,
        vector_search=vector_search,
        sparse_search=sparse_search,
        document_ids=document_ids,
    ...
    partial_matches = [
        item for item in partial if matches_metadata(item, _without_metadata(plan, partial=True))
    ]
    if len(partial_matches) >= 3:
        return partial_matches
    return _retrieve_once(
        _without_metadata(plan),
        database=database,
        vector_search=vector_search,
        sparse_search=sparse_search,
        document_ids=document_ids,
...
def _retry_queries(plan: QueryPlan, missing: Sequence[str]) -> list[str]:
    if plan.subqueries:
...
def run_adaptive_retrieval(
    ...
    *,
    ...
    strict_candidates = _retrieve_once(
        plan,
        database=database,
        vector_search=vector_search,
        sparse_search=sparse_search,
    ...
    if allow_retry and assessment.should_retry:
        retry_queries = _retry_queries(plan, assessment.missing_aspects)
        if progress:
            progress("当前证据覆盖不足，正在补充检索一次")
        retry_hits: list[Evidence] = []
        for retry_query in retry_queries:
            retry_plan = plan.model_copy(
                update={"rewritten_query": retry_query, "subqueries": [], "needs_multi_query": False}
            )
            retry_hits.extend(
                _retrieve_once(
                    _without_metadata(retry_plan, partial=True),
                    database=database,
                    vector_search=vector_search,
                    sparse_search=sparse_search,
                    document_ids=document_ids,
