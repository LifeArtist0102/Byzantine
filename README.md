...
## Evidence-first 自适应混合 RAG

Historia 面向长篇历史与学术文献构建：检索不是单纯“找相似段落”，而是以原始史料为边界，在精准召回、上下文完整性与证据可追溯性之间动态平衡。

1. **结构感知切块**：以 `Section → Parent → Child` 组织文献，而非机械地按固定长度切分。系统保留章节路径、页码、原文区域及上下文关系，并借助本地 BGE-M3 判断语义边界，减少对长篇论证的截断。
2. **多层元数据增强**：人物、地点、时间、主题和事件参与检索与过滤。元数据明确区分 `trusted`、`candidate` 与 `llm_inference`：原始提取结果、待确认候选及模型辅助推断彼此隔离，避免模型推断污染史料本身。
3. **Dense + Sparse + Keyword 混合检索**：BGE-M3 Dense、BGE-M3 Sparse 和 SQLite FTS5 三路召回并行执行，再以 Reciprocal Rank Fusion（RRF）融合排序，兼顾语义相近、术语精确匹配与稀有关键词。
4. **自适应检索**：简单问题先由本地规则分析和改写，无须调用模型；只有复杂、多轮或信息不完整的问题才会触发 LLM Planner。首轮证据不足时，系统识别缺失方面并执行一次补充检索。
5. **动态上下文扩展**：先精确召回最小的 Child Chunk，再按问题类型和令牌预算补充 Parent 或相邻 Chunk，在回答聚焦度与论证上下文之间取得平衡。
6. **Evidence-grounded Generation**：生成模型只能基于检索到的原始文本作答，并强制使用 `[S1]`、`[S2]` 等证据引用；每项证据保留书名、章节、页码及 PDF 原文区域，便于逐条回查并降低幻觉。
